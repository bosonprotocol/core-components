...
// from https://github.com/authereum/eth-revert-reason (MIT Licensed)
...
export async function getRevertReason(
  txHash: string,
  network = "mainnet",
  blockNumber: string | number | undefined = undefined,
  customProvider: ethers.providers.Provider | undefined = undefined
) {
...
async function validateInputPreProvider(txHash: string, network: string) {
...
function getProvider(customProvider: any, network: string) {
  ...
  if (customProvider && customProvider.version) {
    customProvider = new ethers.providers.Web3Provider(
...
function decodeMessage(code: string, network: string) {
